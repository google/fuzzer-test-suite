...
static void test(char *cert, char *ca)
{
    int ret = -1;

    uint8_t *ca_data = NULL, *cert_data = NULL;
    size_t ca_sz, cert_sz;

    if ((ca_sz = read_file(ca, &ca_data)) == -1) {
    ...
    ret = verify_cert_mem(cert_data, cert_sz, ca_data, ca_sz);
    printf("ret: %d\n", ret);
    if (ret) {
      char errorString[80];
...
{
    if (argc != 3) {
        printf("usage: %s server_cert ca_cert\n", argv[0]);
        return EXIT_SUCCESS;
    }
